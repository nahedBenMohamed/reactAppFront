...
import { action_evaluation_getGrammars } from '../../../store/actions';
import { selectAnalysisResult, selectGrammars } from '../../../store/reducers/evaluation.reducers';
...
const GrammarComponent = ({ t, scores }) => {
	...
	useEffect(() => {
		if (grammar[indexGrammer]?.min_occur > selectionCount && indexGrammer < grammar?.length - 1) {
	...
	useEffect(() => {
		if (indexGrammer === grammar?.lenght - 1) return setReachMax(true);
		setReachMax(false);
	}, [indexGrammer]);
	...
	return (
		<div className="tabs-panel is-active" id="grammar">
			...
			<div className="cell">
				<textarea placeholder="Weitere Beobachtungen/Auffälligkeiten" id="grammar_notes"></textarea>
			</div>
